...
private extern (C) @system nothrow @nogc
{
    ...
    void Clay_SetQueryScrollOffsetFunction(
        Clay_QueryScrollOffsetFn queryScrollOffsetFunction, void* userData);
...
{
    Clay_SetMeasureTextFunction(measureTextFunction, userData);
}

void claySetQueryScrollOffsetFunction(
    Clay_QueryScrollOffsetFn queryScrollOffsetFunction, void* userData)
{
    Clay_SetQueryScrollOffsetFunction(queryScrollOffsetFunction, userData);
}
...
void claySetMaxMeasureTextCacheWordCount(int c)
{
    Clay_SetMaxMeasureTextCacheWordCount(c);
}
...
{
    return Clay__HashStringWithOffset(key, offset, seed);
}
...
{
    return Clay__GetParentElementId();
}
